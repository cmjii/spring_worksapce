...
public class FileHandler {
	...
	public List<FileVO> uploadFiles(MultipartFile[] files){
		...
		//오늘날짜(today)의 폴더 구성
		today = today.replace("-", File.separator); //파일 경로 표시로 바꿔주세요.
		...
		for(MultipartFile file : files) {
			...
			String onlyFileName = originalFileName.substring(originalFileName.lastIndexOf(File.separator)+1); //파일 경로가 있다면 실 파일명만 추출
			fvo.setFile_name(onlyFileName);
			
			//uuid 생성
			UUID uuid = UUID.randomUUID(); //UUID는 객체가 있음 (같은 파일을 구분하기 위해서)
